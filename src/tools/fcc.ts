...
export function registerFcc(server: McpServer) {
  server.tool(
    "search_fcc",
    ...
    async ({ fccId, granteeCode, applicantName, limit }) => {
      ...
      if (fccId) params.set("fccId", fccId);
      if (granteeCode) params.set("granteeCode", granteeCode);
      if (applicantName) params.set("applicantName", applicantName);
