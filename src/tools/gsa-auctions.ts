...
export function registerGsaAuctions(server: McpServer) {
  server.tool(
    "search_gsa_auctions",
    ...
    async ({ query, state, agency, status, min_bid, max_bid, limit, offset }) => {
      ...
      if (min_bid != null) params.set("min_bid", String(min_bid));
      if (max_bid != null) params.set("max_bid", String(max_bid));
