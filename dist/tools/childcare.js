...
export function registerChildcare(server) {
    server.tool("childcare_cost", "Look up childcare costs by ZIP code, county, or state. Uses DOL National Database of Childcare Prices (48K records). Returns weekly/annual prices for center-based and family childcare by age group.", {
        ...
        if (zip)
            params.set("zip", zip);
        if (county_fips)
            params.set("county_fips", county_fips);
        if (state)
    ...
    server.tool("childcare_states", "Get childcare cost averages for all US states. Compare center-based and family childcare pricing across the country by age group.", {
        year: z.number().int().optional().describe("Data year (default: latest available)"),
    }, async ({ year }) => {
        const params = new URLSearchParams();
        if (year)
    ...
    server.tool("childcare_compare", "Compare childcare costs side-by-side across multiple ZIP codes or locations. Compare up to 10 locations at once.", {
