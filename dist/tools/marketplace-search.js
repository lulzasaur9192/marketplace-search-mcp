...
export function registerMarketplaceSearch(server) {
    server.tool("marketplace_search", `Search across 13 online marketplaces. Supported marketplaces: ${MARKETPLACES.join(", ")}. Returns listings with prices, conditions, seller info, and URLs. Each marketplace specializes in different categories: tcgplayer (trading cards), reverb (music gear), grailed (designer fashion), redfin (real estate), bonanza (general), thriftbooks/abebooks/goodreads (books), offerup/craigslist (local classifieds), swappa (electronics), stubhub (event tickets), poshmark (fashion resale).`, {
        marketplace: z
            .enum(MARKETPLACES)
